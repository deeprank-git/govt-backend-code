"""
Dump Drishti IAS scraped articles into the MongoDB CurrentAffairs collection.

Usage:
    python drishti_dump.py [path/to/drishti_articles.json]

Reads from drishti_articles.json in the same directory by default.
...
DEFAULT_JSON = Path(__file__).parent / "drishti_articles.json"
...
def dump(json_path: Path) -> None:
    ...
    for article in today_articles:
        ...
        ops.append(
            UpdateOne(
                {"url": url},
                {"$set": doc, "$setOnInsert": {"createdAt": doc["updatedAt"], "views": 0}},
                upsert=True,
            )
        )
    ...
    print(
        f"[OK] Drishti IAS — {result.upserted_count} inserted, "
